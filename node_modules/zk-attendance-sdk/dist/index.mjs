...
var USHRT_MAX = 65535;
var MAX_CHUNK = 65472;
var ERROR_ACK_COMMAND_IDS = /* @__PURE__ */ new Set([
  COMMANDS.CMD_ACK_ERROR,
  COMMANDS.CMD_ACK_UNAUTH,
  COMMANDS.CMD_ACK_UNKNOWN,
...
var decodeRecordRealTimeLog18 = (recordData) => {
  const userId = recordData.readUIntLE(8, 4);
...
var isEventTCP = (data, logger = noopLogger) => {
  try {
    ...
    logger.error(`[EVENT_DETECT_TCP] ${String(err)} ,${data.toString("hex")}`);
    return false;
  }
};
var isEventUDP = (data, logger = noopLogger) => {
  if (data.length < 8) return false;
  try {
    const commandId = decodeUDPHeader(data.subarray(0, 8)).commandId;
    return commandId === COMMANDS.CMD_REG_EVENT;
  } catch (err) {
    logger.error(`[EVENT_DETECT_UDP] ${String(err)} ,${data.toString("hex")}`);
    return false;
  }
};
...
// src/helper/jtcp.ts
var JTCP = class {
  constructor(ip, port, timeout, logger = noopLogger) {
    this.ip = ip;
    this.port = port;
  ...
  executeCmd(command, data) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (sanitizedReply.length >= 2) {
          const replyCommandId = sanitizedReply.readUInt16LE(0);
          if (ERROR_ACK_COMMAND_IDS.has(replyCommandId)) {
            throw new Error(
              `Device rejected command ${exportErrorMessage(command)}: ${exportErrorMessage(replyCommandId)}`
            );
          }
        }
  ...
  requestData(msg) {
    return new Promise((resolve, reject) => {
      ...
      const requestTimeout = Math.max(this.timeout ?? 0, 1e4);
      ...
      const handleOnData = (data) => {
        ...
        if (isEventTCP(data, this.logger)) return;
        if (timer) {
          clearTimeout(timer);
        }
  ...
  sendChunkRequest(start, size) {
    ...
    socket.write(buffer, (err) => {
      if (err) {
        this.logger.error(`[TCP][SEND_CHUNK_REQUEST] ${err.toString()}`);
      }
    });
  }
      ...
      switch (header.commandId) {
        ...
        case COMMANDS.CMD_PREPARE_DATA: {
          ...
          const handleOnData = (incoming) => {
            if (isEventTCP(incoming, this.logger)) {
              return;
            }
            if (timer) {
              clearTimeout(timer);
  ...
  async getRealTimeLogs(cb = () => {
    ...
    const handleData = (incoming) => {
      ...
      while (tcpBuffer.length >= 8) {
        ...
        if (frame.length < 16 || !isEventTCP(frame, this.logger)) {
          continue;
        }
...
var JUDP = class {
  constructor(ip, port, timeout, inboundPort, logger = noopLogger) {
    this.ip = ip;
    this.port = port;
    this.timeout = timeout;
  ...
  executeCmd(command, data) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (reply.length >= 2) {
          const replyCommandId = reply.readUInt16LE(0);
          if (ERROR_ACK_COMMAND_IDS.has(replyCommandId)) {
            throw new Error(
              `Device rejected command ${exportErrorMessage(command)}: ${exportErrorMessage(replyCommandId)}`
            );
  ...
  requestData(msg) {
    return new Promise((resolve, reject) => {
      ...
      const handleOnData = (data) => {
        if (isEventUDP(data, this.logger)) {
          return;
        }
        if (timer) {
          clearTimeout(timer);
  ...
  sendChunkRequest(start, size) {
    ...
    socket.send(buffer, 0, buffer.length, this.port, this.ip, (err) => {
      if (err) {
        this.logger.error(`[UDP][SEND_CHUNK_REQUEST] ${err.toString()}`);
      }
    });
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      switch (header.commandId) {
        ...
        case COMMANDS.CMD_PREPARE_DATA: {
          ...
          const handleOnData = (incoming) => {
            if (isEventUDP(incoming, this.logger)) {
              return;
            }
            resetTimer(() => {
              internalCallback(
  ...
  async getRealTimeLogs(cb = () => {
    ...
    this.isRealtimeListening = true;
    const handleMessage = (data) => {
      if (!isEventUDP(data, this.logger) || data.length < 16) {
        return;
      }
      ...
      socket.send(ack, 0, ack.length, this.port, this.ip, (err) => {
        if (err) {
          this.logger.error(`[UDP][ACK_EVENT] ${err.message}`);
        }
      });
...
var ZKAttendanceClient = class {
  constructor(ip, port = 4370, timeout = 5e3, inboundPort, logger = noopLogger) {
    this.ip = ip;
    this.port = port;
    this.timeout = timeout;
    ...
    this.jtcp = new jtcp_default(this.ip, this.port, this.timeout, logger);
    this.judp = new judp_default(
      this.ip,
      this.port,
      this.timeout,
      resolvedInboundPort,
