...
declare const noopLogger: ZKLogger;
declare const fileLogger: ZKLogger;
...
declare class JTCP {
    ...
    private replyId;
    private socket;
    private isRealtimeListening;
    constructor(ip: string, port: number, timeout?: number | undefined, logger?: ZKLogger);
...
declare class JUDP {
    ...
    private replyId;
    private isRealtimeListening;
    constructor(ip: string, port: number, timeout: number | undefined, inboundPort: number, logger?: ZKLogger);
...
type ZKUserTcp = ReturnType<typeof decodeUserData72>;
type ZKUserUdp = ReturnType<typeof decodeUserData28>;
type ZKAttendanceRecordTcp = ReturnType<typeof decodeRecordData40> & {
    ip: string;
};
type ZKAttendanceRecordUdp = ReturnType<typeof decodeRecordData16> & {
    ip: string;
};
type ZKRealTimeLogTcp = ReturnType<typeof decodeRecordRealTimeLog52>;
type ZKRealTimeLogUdp = ReturnType<typeof decodeRecordRealTimeLog18>;
...
declare class ZKAttendanceClient {
    ...
    private timer;
    private isBusy;
    constructor(ip: string, port?: number, timeout?: number, inboundPort?: number, logger?: ZKLogger);
...
export { type ZKAttendanceRecordTcp, type ZKAttendanceRecordUdp, type ZKDeviceInfo, type ZKLogger, type ZKRealTimeLogTcp, type ZKRealTimeLogUdp, type ZKUserTcp, type ZKUserUdp, ZKAttendanceClient as default, fileLogger, noopLogger };
