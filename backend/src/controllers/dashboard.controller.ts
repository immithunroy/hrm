...
import { isDeviceConnected } from '../services/zktService';

const DHAKA_OFFSET_MS = 6 * 3600 * 1000;

// Dhaka calendar day (YYYY-MM-DD) for a date.
const dhakaDay = (value: Date): string => {
...
export const getDashboard = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // ----- Overview counts -----
    const [totalEmployees, activeEmployees, departmentsCount, openPositions, pendingLeave] =
    ...
    const isTodayHoliday = await isMarkedHoliday(todayStr);

    const todayByEmployee = new Map<string, any>();
    todaysRecords.forEach((r) => {
      const empId = r.employeeId;
      const cur = todayByEmployee.get(empId);
      if (!cur || (cur.checkIn?.getTime() || 0) > (r.checkIn?.getTime() || 0)) todayByEmployee.set(empId, r);
    });
    ...
    for (const emp of activeEmployeesList) {
      if (emp.attendanceExempt) continue; // admin/co-founder staff do not punch
      if (isWeeklyHoliday(todayStr, emp.weeklyHoliday || 'FRIDAY')) {
        weeklyHolidayToday++;
        continue;
      }
      if (isTodayHoliday) {
      ...
      if (!rec) {
        ...
        presentToday++;
        if (rec.status === 'EARLY') earlyToday++;
      } else {
        absentToday++;
      }
    }
    ...
    // ----- 30-day trend -----
    const trendDays: Array<{ date: string; present: number; late: number; early: number; absent: number; expected: number }> = [];
    ...
    const holidaySet30 = await prisma.holiday.findMany({
      where: { date: { gte: new Date(trendStartMs), lte: todayEnd } },
      select: { date: true }
    });
    const holidayStrSet = new Set(holidaySet30.map((h) => dhakaDayString(h.date)));
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const dayRecords = trendRecords.filter(
        (r) => r.date.getTime() >= dayStart.getTime() && r.date.getTime() <= dayEnd.getTime()
      );
      ...
      if (!holidayStrSet.has(dayStr)) {
        for (const emp of activeEmployeesList) {
          if (emp.attendanceExempt) continue; // exempt staff not expected to punch
          if (isWeeklyHoliday(dayStr, emp.weeklyHoliday || 'FRIDAY')) continue;
          expected++;
        }
      }

      for (const rec of dayRecords) {
        if (exemptIds.has(rec.employeeId)) continue;
        if (seen.has(rec.employeeId)) continue;
    ...
    monthRecords.forEach((r) => {
      if (exemptIds.has(r.employeeId)) return;
    ...
    const holidaySet = new Set(holidaysThisMonth.map((h) => dhakaDayString(h.date)));
    const holidayNameByDay: Record<string, string> = {};
    holidaysThisMonth.forEach((h) => {
      holidayNameByDay[dhakaDayString(h.date)] = h.name;
    });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      for (const emp of activeEmployeesList) {
        if (emp.attendanceExempt) continue; // exempt staff not expected to punch
        if (isWeeklyHoliday(dayStr, emp.weeklyHoliday || 'FRIDAY')) {
          weeklyHolidayCount++;
        } else {
          expected++;
        }
      ...
      calendar.push({
        ...
        weekday: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][
          new Date(Date.UTC(monthYear, monthNum - 1, day)).getUTCDay()
        ],
        isHoliday,
    ...
    const approvedLeaveDays = await prisma.leaveRequest.aggregate({
    ...
    res.status(200).json({
      ...
      data: {
        ...
        holidays: holidaysThisMonth.map((h) => ({ id: h.id, date: dhakaDayString(h.date), name: h.name })),
        calendar,
        recentAttendance,
        settings
      }
