...
exports.round2 = exports.buildPayslipPdf = exports.buildPayslipWorkbook = exports.buildAttendancePdf = exports.buildAttendanceWorkbook = exports.getLeaveSummary = exports.attendanceStatusLabel = exports.ATTENDANCE_STATUS_LABELS = exports.fmtHM = exports.fmtDhakaWeekdayShort = exports.fmtDhakaWeekday = exports.fmtDhakaDate = exports.fmtDhaka = void 0;
...
const fmtDhaka = (value, withSeconds = false) => {
    if (!value)
        return '—';
...
exports.fmtDhaka = fmtDhaka;
const fmtDhakaDate = (value) => {
    if (!value)
        return '—';
    const d = new Date(new Date(value).getTime() + DHAKA_OFFSET_MS);
    const pad = (n) => String(n).padStart(2, '0');
...
exports.fmtDhakaDate = fmtDhakaDate;
const WEEKDAY_NAMES = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
const WEEKDAY_SHORT = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
// Dhaka weekday name for a date (e.g. "Thursday").
const fmtDhakaWeekday = (value) => {
    if (!value)
        return '—';
    const d = new Date(new Date(value).getTime() + DHAKA_OFFSET_MS);
    return WEEKDAY_NAMES[d.getUTCDay()];
};
exports.fmtDhakaWeekday = fmtDhakaWeekday;
const fmtDhakaWeekdayShort = (value) => {
    if (!value)
        return '—';
    const d = new Date(new Date(value).getTime() + DHAKA_OFFSET_MS);
    return WEEKDAY_SHORT[d.getUTCDay()];
};
exports.fmtDhakaWeekdayShort = fmtDhakaWeekdayShort;
...
const attendanceStatusLabel = (status) => exports.ATTENDANCE_STATUS_LABELS[status] || status;
exports.attendanceStatusLabel = attendanceStatusLabel;
...
const getLeaveSummary = async (employeeId, year) => {
    const where = {
        employeeId_year: { employeeId, year }
    };
    const balance = (await database_1.prisma.leaveBalance.findUnique({ where })) ||
        (await database_1.prisma.leaveBalance.upsert({
            where,
            update: {},
            create: {
                employeeId,
    ...
    const sumUsed = async (leaveType) => {
        const agg = await database_1.prisma.leaveRequest.aggregate({
            where: {
                employeeId,
                leaveType: leaveType,
                status: 'APPROVED',
...
const buildAttendanceWorkbook = async (rows, title, leaveByEmployee, payrollSettings) => {
    const workbook = new exceljs_1.default.Workbook();
    workbook.creator = 'ZKT Payroll HR System';
    ...
    for (const g of groupByEmployee(rows)) {
        ...
        for (const r of g.rows) {
            ...
            [
                (0, exports.fmtDhakaDate)(r.date),
                (0, exports.fmtDhakaWeekday)(r.date),
                (0, exports.fmtDhaka)(r.checkIn),
                (0, exports.fmtDhaka)(r.checkOut),
                (0, exports.fmtHM)((r.breakMinutes || 0) / 60),
                (0, exports.fmtHM)(r.workHours),
                (0, exports.fmtHM)(r.overtimeHours),
                (0, exports.fmtHM)(r.earlyOvertimeHours),
                r.lateMinutes ?? '',
                (0, exports.attendanceStatusLabel)(r.status || '')
            ].forEach((v, i) => {
        ...
        ['Total', '', '', '', (0, exports.fmtHM)(sumErrand), (0, exports.fmtHM)(sumWork), (0, exports.fmtHM)(sumOT), (0, exports.fmtHM)(sumEarlyOT), Math.round(sumLate), ''].forEach((v, i) => {
...
exports.buildAttendanceWorkbook = buildAttendanceWorkbook;
const buildAttendancePdf = (rows, title, leaveByEmployee, payrollSettings) => {
    return new Promise((resolve) => {
        // All margins narrow (28pt).
        const doc = new pdfkit_1.default({ size: 'A4', layout: 'portrait', bufferPages: true, margins: { top: 28, left: 28, right: 28, bottom: 28 } });
        const chunks = [];
        ...
        for (const g of groups) {
            ...
            doc.moveDown(0.1);
            doc.font('Helvetica').fontSize(9).text(`Department: ${g.employee?.department?.name || ''}  |  ID: ${g.employee?.employeeId || ''}`, { align: 'left' });
            doc.moveDown(0.15);
            // Single-row summary: late days, total late minutes, total errand minutes, on-time days
            doc.font('Helvetica-Bold').fontSize(9).text(`Late days: ${s.lateDays}  |  Total late minutes: ${s.totalLateMinutes}  |  Total errand minutes: ${s.totalErrandMinutes}  |  On-time days: ${s.onTimeDays}`, { align: 'left' });
            doc.font('Helvetica');
            doc.moveDown(0.15);
            ...
            if (payrollSettings) {
                ...
                const holidayHourlyOT = hourlyBasic * payrollSettings.holidayOvertimeRate;
                const currency = payrollSettings.currency || 'BDT';
                doc.font('Helvetica-Bold').fontSize(9).text(`Hourly Basic Rate: ${currency} ${hourlyBasic.toFixed(2)}  |  Hourly OT Rate: ${currency} ${hourlyOT.toFixed(2)}  |  Holiday OT Hourly Rate: ${currency} ${holidayHourlyOT.toFixed(2)}`, { align: 'left' });
                doc.font('Helvetica');
                doc.moveDown(0.15);
            }
            ...
            const table = [ATTENDANCE_HEADERS];
            let sumErrand = 0;
            let sumWork = 0;
            let sumOT = 0;
            let sumEarlyOT = 0;
            ...
            for (const r of g.rows) {
                ...
                table.push([
                    (0, exports.fmtDhakaDate)(r.date),
                    (0, exports.fmtDhakaWeekdayShort)(r.date),
                    (0, exports.fmtDhaka)(r.checkIn),
                    (0, exports.fmtDhaka)(r.checkOut),
                    (0, exports.fmtHM)((r.breakMinutes || 0) / 60),
                    (0, exports.fmtHM)(r.workHours),
            ...
            table.push([
                'Total', '', '', '',
                (0, exports.fmtHM)(sumErrand),
                (0, exports.fmtHM)(sumWork),
                (0, exports.fmtHM)(sumOT),
                (0, exports.fmtHM)(sumEarlyOT),
                Math.round(sumLate),
                ''
            ]);
...
exports.buildAttendancePdf = buildAttendancePdf;
const drawTable = (doc, rows, widths, headerRows) => {
    const startX = doc.page.margins.left;
    let y = doc.y;
    const rowHeight = 18;
    const fontSize = 8;
    const headerSet = new Set(headerRows ?? [0]);
    const drawRow = (cells, isHeader, yPos) => {
        let x = startX;
...
const buildPayslipWorkbook = async (data) => {
    const wb = new exceljs_1.default.Workbook();
    wb.creator = 'ZKT Payroll HR System';
    ...
    const drawPayslipBlock = (startRow, label) => {
        let r = startRow;
        // Label
        ...
        const baseEarnings = data.salary.salaryType === 'SCALED'
            ? [
...
exports.buildPayslipWorkbook = buildPayslipWorkbook;
const buildPayslipPdf = (data) => {
    return new Promise((resolve) => {
        ...
        const drawPayslip = (label, yStart) => {
            let y = yStart;
            ...
            doc.font('Helvetica-Bold').fontSize(8).fillColor('#000000')
                .text(`Late days: ${data.totals.lateCount}  |  Total late minutes: ${data.totals.totalLateMinutes}  |  Total errand minutes: ${data.totals.errandMinutes ?? 0}  |  On-time days: ${data.totals.presentDays}`, lx, y, { width: pageWidth });
            y += 11;
            // Payment rates line (same as attendance report)
            {
                ...
                doc.font('Helvetica-Bold').fontSize(8)
                    .text(`Hourly Basic Rate: ${cur} ${hourlyBasic.toFixed(2)}  |  Hourly OT Rate: ${cur} ${hourlyOT.toFixed(2)}  |  Holiday OT Hourly Rate: ${cur} ${holidayHourlyOT.toFixed(2)}`, lx, y, { width: pageWidth });
                y += 13;
            }
        ...
        doc.moveDown(0.3);
        // Summary line
        doc.font('Helvetica-Bold').fontSize(9).text(`Late days: ${data.totals.lateCount}  |  Total late minutes: ${data.totals.totalLateMinutes}  |  Total errand minutes: ${data.totals.errandMinutes ?? 0}  |  On-time days: ${data.totals.presentDays}`, { align: 'left' });
        doc.moveDown(0.3);
        // Payment rates
        {
            const salary = data.salary.basic;
            ...
            const hourlyOT = hourlyBasic * (data.salary.overtimeRate ?? 1.5);
            const holidayHourlyOT = hourlyBasic * (data.salary.holidayOvertimeRate ?? 2);
            doc.font('Helvetica-Bold').fontSize(9).text(`Hourly Basic Rate: ${cur} ${hourlyBasic.toFixed(2)}  |  Hourly OT Rate: ${cur} ${hourlyOT.toFixed(2)}  |  Holiday OT Hourly Rate: ${cur} ${holidayHourlyOT.toFixed(2)}`, { align: 'left' });
            doc.moveDown(0.3);
        }
        // Leave and Attendance Details
        ...
        const attTable = [ATTENDANCE_HEADERS];
        let sumErrand = 0;
        let sumWork = 0;
        let sumOT = 0;
        let sumEarlyOT = 0;
...
exports.buildPayslipPdf = buildPayslipPdf;
